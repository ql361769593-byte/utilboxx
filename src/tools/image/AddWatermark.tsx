    ...
    <div className="space-y-4">
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm font-medium block mb-1">{dict.ui.position}</label>
            <select value={position} onChange={(e) => setPosition(e.target.value as any)} className="w-full border border-slate-300 rounded px-3 py-2">
              <option value="tl">{dict.ui.pos_tl}</option>
              <option value="tr">{dict.ui.pos_tr}</option>
              <option value="bl">{dict.ui.pos_bl}</option>
              <option value="br">{dict.ui.pos_br}</option>
              <option value="center">{dict.ui.pos_center}</option>
            </select>
          </div>
          <div>
            <label className="text-sm font-medium block mb-1">{dict.ui.size}: {size}px</label>
            <input type="range" min="12" max="120" value={size} onChange={(e) => setSize(parseInt(e.target.value))} className="w-full" />
          </div>
        </div>
        <div>
          <label className="text-sm font-medium block mb-1">{dict.ui.opacity}: {(opacity * 100).toFixed(0)}%</label>
      ...
      <FileTool
        ...
        onProcess={async (files) => {
          ...
          if (blob) downloadBlob(blob, `watermarked-${file.name}`);
