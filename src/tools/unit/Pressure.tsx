...
const UNITS = [
  ...
  { key: "psi", name: "PSI (psi)", factor: 6894.757293168 },
  { key: "atm", name: "Atmosphere (atm)", factor: 101_325 },
  { key: "mmHg", name: "mmHg / Torr", factor: 133.322387415 },
  { key: "torr", name: "Torr", factor: 133.322368421 },
];

export default function Pressure({ dict }: { dict: Dictionary }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_220px] gap-3 items-end">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium block">{dict.tools.unit.pressure}</label>
          <select
            value={src.key}
          ...
          {UNITS.filter((u) => u.key !== src.key).map((u) => (
            ...
            >
              ...
              <span className="font-mono text-sm font-semibold text-slate-900">
                {isFinite(base / u.factor) ? (base / u.factor).toLocaleString(undefined, { maximumFractionDigits: 6 }) : "—"}
              </span>
            </div>
          ))}
        </div>
