...
Scanned PDFs are made of images rather than text, so they behave differently. A 300 DPI scan of a 20-page contract can easily be 50 MB. The good news is that image-heavy PDFs usually shrink the most — anywhere from **30% to 70%** size reduction is common. For best results, look for a tool that runs OCR (optical character recognition) and saves the result as searchable, compressed text plus lower-resolution page images..
...
スキャン PDF は元が画像なので、振る舞いが異なります。300 DPI でスキャンした 20 ページの契約書は簡単に 50 MB に達します。良いニュースは、画像主体の PDF こそ圧縮率が大きく、**30%〜70%** のサイズ削減が珍しくないことです。ベストな結果を出したいなら、OCR（光学文字認識）付きのツールを選び、結果を「検索可能なテキスト + 低解像度ページ画像」として保存するのが理想です。UtilBoxx の圧縮ツールはスキャン PDF にも強く、画像ストリームに対して自動的に適切な設定を選んでくれます。
...
Gescannte PDFs bestehen aus Bildern statt aus Text und verhalten sich daher anders. Ein mit 300 DPI gescannter 20-seitiger Vertrag bringt es mühelos auf 50 MB. Die gute Nachricht: Bildlastige PDFs schrumpfen meistens am stärksten — Reduktionen zwischen **30 % und 70 %** sind keine Seltenheit. Für die besten Ergebnisse wählen Sie ein Werkzeug, das OCR (optische Zeichenerkennung) durchführt und das Ergebnis als durchsuchbaren Text plus niedriger aufgelöste Seitenbilder speichert. Der Kompressor von UtilBoxx behandelt gescannte PDFs sehr gut und wählt automatisch sinnvolle Einstellungen für Bilddatenströme.
